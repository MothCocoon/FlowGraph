...
void SFlowPaletteItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData)
{
	...
	if (FFlowGraphSpawnCommands::IsRegistered())
	{
		if (GraphAction->GetTypeId() == FFlowGraphSchemaAction_NewNode::StaticGetTypeId())
		{
			UClass* FlowNodeClass = StaticCastSharedPtr<FFlowGraphSchemaAction_NewNode>(GraphAction)->FlowNodeClass;
			HotkeyChord = FFlowGraphSpawnCommands::Get().GetChordByClass(FlowNodeClass);
		}
		else if (GraphAction->GetTypeId() == FFlowGraphSchemaAction_NewComment::StaticGetTypeId())
		{
			HotkeyChord = FFlowGraphSpawnCommands::Get().GetCommentChord();
		}
	}
